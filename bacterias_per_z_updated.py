...
if not all_filtered_z:
    ...
    bin_edges = [0, 2, 4, 6, 8, 10, 12, 14, 20]  # last bin is 14–20 μm
    bin_ranges = list(zip(bin_edges[:-1], bin_edges[1:]))

    # Make sure counts_per_bin matches the number of bins
    # If you use numpy.histogram or manual binning, make sure to align them
    # You may already have counts_per_bin defined; ensure it's the right length
    # For this example, let's assume it's already calculated correctly
    # Example dummy data:
    # counts_per_bin = [5, 10, 8, 12, 7, 3, 2, 1]  # len must be len(bin_ranges)

    # --- Create inset plot ---
    fig, ax = plt.subplots(figsize=(10, 6))
    ax_inset = ax.inset_axes([0.6, 0.5, 0.35, 0.45])  # Adjust inset position

    # Plot horizontal lines per bin
    for (x_start, x_end), yi in zip(bin_ranges, counts_per_bin):
        ax_inset.hlines(y=yi, xmin=x_start, xmax=x_end, colors='blue', linewidth=2)

    # Add exponential fit if available
    if plot_fit:
        ax_inset.plot(x_fit, y_fit, color='red', linestyle='--')
    ...
    # Set x-ticks at bin starts (optional: midpoints or edges)
    ax_inset.set_xticks(bin_edges)
