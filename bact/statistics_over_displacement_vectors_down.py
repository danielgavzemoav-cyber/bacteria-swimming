...
def visualize_velocity_field(x_all, y_all, u_rel_mean, v_rel_mean, mean_velocity):
    ...
    plt.quiver(
        ...
        u_rel_mean,
        v_rel_mean,
        scale=0.8,  # Adjust scale for better appearance
        angles='xy',
        scale_units='xy',
        color='blue',
...
measure_dir = r'down2\Default'
cropped_dir = r'down2\Cropped\bac_id_0'
vectors_dir = r'down2\Cropped\bac_id_0'
output_dir = r'down2\bac_id_0'
frames = [int(d.split('_')[-2][4:]) for d in os.listdir(cropped_dir) if d.endswith('tif')]
frames_missed = find_missing_frames_sorted(frames, 299)  # zero-based
measure_file = os.path.join(measure_dir, 'metadata.txt')
vectors_file = os.path.join(vectors_dir, 'PIVlab.mat')
