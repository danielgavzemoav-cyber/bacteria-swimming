...
with open(r'up2\track_spots.csv', 'r') as f:
    ...
    for row in rows:
        ...
        # if tid in ignore_track_ids_down:
        #     continue
        frame = int(row['FRAME'])
        if frame not in tids_by_frames:
            tids_by_frames[frame] = [(tid, float(row['POSITION_X']), float(row['POSITION_Y']))]
        else:
            tids_by_frames[frame].append((tid, float(row['POSITION_X']), float(row['POSITION_Y'])))
    ...
    double_dots_by_frames = {frame: tids_by_frames[frame] for frame in tids_by_frames if len(tids_by_frames[frame]) > 1}


for bac_id, bac_tracks in enumerate(tracks):
    plt.figure()
    for tid, track in bac_tracks.items():
        xs = track[0]
        ys = track[1]
        frames = track[2]
        plt.scatter(xs, ys, label=f"track id {tid}")
    for frame in double_dots_by_frames:
        xs = []
        ys = []
        for dot in double_dots_by_frames[frame]:
            xs.append(dot[1])
            ys.append(dot[2])
...
for bac_id, bac_t in single_bac_tracks.items():
    ...
    #     last_frame = frame
    #
    images_dir = r'down2\Default'
    images_out_dir = fr'down2\Cropped\bac_id_{bac_id}'
    ...
    for image in images:
        ...
        if x == 0 or y == 0:
            print('Haaaa????')
        cropped, is_cutted = crop_around(img, x, y, 301)
        if is_cutted:
